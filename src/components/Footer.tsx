      ...
      <div className="bg-black py-8 px-8 xl:p-3xl xl:px-6xl text-white flex flex-col md:flex-row justify-between gap-8 pb-4">
        <div className="flex flex-col gap-y-8">
          ...
          <div className="flex flex-row gap-3 justify-center xl:justify-start">
            <img
              ...
              alt="Logo e-UMKM"
              className="pr-3 h-[50px] lg:h-[60px]"
            />
            <div className="flex flex-col text-white font-bold justify-center gap-4 text-sm md:text-base">
              <h1>{common("pemkab")}</h1>
              <h1>KULON PROGO</h1>
            </div>
          </div>
          {/* logo end */}
          <div className="copyright text-center text-xs md:text-sm md:text-left">
        ...
        <div className="flex-row hidden lg:flex">
          <div className="flex flex-col gap-y-1">
            ...
            {menuItems.map((item) => (
              <div key={item.slug}>
                <a href={item.href} className="hover:text-warning text-sm">
                  {common(item.slug)}
                </a>
              </div>
            ))}
        ...
        <div className="flex flex-row xl:gap-10">
          ...
          <div className="flex flex-col gap-y-5 items-center md:items-start w-full">
            <h1 className="font-bold pb-4 text-sm md:text-base xl:text-lg text-center md:text-left">
              {common("contact")}
            </h1>
            <p className="max-w-[18rem] text-center md:text-left text-xs md:text-sm">
