...
const TableModalFilter: FC<TableModalFilterProps> = ({
  ...
  return (
    <motion.div
      ...
      className="fixed xl:relative box-filter bg-white flex flex-col shadow-lg xl:shadow-sm py-8 px-6 rounded-e-sm xl:rounded-sm text-sm md:text-base dark:bg-black z-[60] w-[20rem] md:w-[25rem] max-h-[80vh] overflow-y-scroll xl:max-h-fit xl:overflow-y-hidden border xl:border-none"
    >
      <div className="box absolute top-0 right-0 transform -translate-x-3 translate-y-2">
        <ChevronDown
          className={`w-7 h-7 xl:w-8 xl:h-8 p-1 bg-silver text-black dark:bg-black dark:border dark:text-white transform hover:bg-inactive hover:text-accent5 rounded-full cursor-pointer transition duration-300 ${showFilter ? "transform rotate-90" : ""
            }`}
        ...
        <div className="flex flex-col gap-4 ">
          <div className="">
            ...
            >
              ...
              <FontAwesomeIcon
                icon={faChevronUp}
                className={`hover:text-grey transform ${!showInstitutionFilter && "rotate-180"
                  }`}
              />
            </div>
            <AnimatePresence>
                ...
                >
                  <div className="box w-full mt-2">
                    <div className="flex flex-row justify-between py-1">
                      <p className="text-xs lg:text-sm text-grey hover:text-grey/50 transition duration-300 cursor-pointer dark:text-white dark:hover:text-grey" onClick={() => setInstitusionFilter(institusionTypeData)
                      }>
                        Pilih semua
                      </p>
